...
5. Menus/new-game/pause and replayed new-game controls pass the [menu milestone](menus.md), with three live recordings. The new [save/load core](save-load.md) passes 90 named checks and separate-process continuation through intermission into E1M2; the inherited serializer remains unused. Next, connect save slots to menus, show overwrite/load decisions and source mismatches, and qualify worker/asset handoff plus replayed loads. Then integrate automap and settings.
