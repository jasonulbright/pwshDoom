...
| M3 — Audio | PowerShell-controlled effects/music and declared device backend | Offline output correctness, real playback review, underrun/latency/load measurements | [Opt-in PowerShell effects and Windows playback](audio.md) integrated; volume/mute and full-host PCM identity verified; [dry PowerShell music](music-synthesis.md) preserves the complete E1M1 loop byte for byte after numeric optimization, with 55 synthesis tests; new full fixture costs 143 seconds for 98 seconds of audio versus earlier 381/664-second observations; bounded parallel synthesis is next to investigate, with effects/live integration, fidelity and audible latency still open |
