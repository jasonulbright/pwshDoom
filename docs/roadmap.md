...
| M3 — Audio | PowerShell-controlled effects/music and declared device backend | Offline output correctness, real playback review, underrun/latency/load measurements | [Opt-in PowerShell effects and Windows playback](audio.md) integrated; full-host replay/pause/resize PCM identity verified; persistent volume/mute verified; music, audible latency and broader qualification pending |
...
| Audio implementation routes | PowerShell mixer with a tested waveOut device experiment | Separate WAD event/decode/mix work, synthesis, device playback, fidelity, licensing, and cost; normal host still silent |
