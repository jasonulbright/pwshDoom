...
A focused correctness check then exposed a separate bug in `VisibilityCheck.InterceptVector`: its denominator-zero branch compared Fixed wrappers instead of their numeric `.Data` values. Distinct zero-valued wrappers were treated as unequal, so parallel lines yielded `Int32.MinValue` instead of zero. The one-line comparison fix passes the two direct intercept cases and all four existing E1M1–E1M4 fixed-route regressions through their expected next maps. See [the numeric visibility investigation](numeric-visibility.md#denominator-zero-correction) and the paired receipts `results/visibility-intercept-before-fix.json` / `results/visibility-intercept-after-fix.json`. This correction closes the identified correctness issue; broader sight fidelity and performance investigation remains open.
