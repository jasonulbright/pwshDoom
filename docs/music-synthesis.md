...
`music-numeric-validation.json` passes 68 evidence checks, including all 55 synthesis checks, retained short-stage WAV equality, full-score equality and current source hashes/parses. The remaining practical question is whether further control optimization or a bounded PowerShell synthesis worker pool can keep enough audio ahead of playback. Any parallel experiment must measure startup, memory, output differences from changed accumulation order, pause/transition behavior and sustained production under gameplay load before host adoption. Preserve all score-event boundaries in each worker, even when its channel subset ignores an event, so release cleanup and caller partitions remain comparable. Compare grouped serial and parallel accumulation against the existing full-voice order; do not assume floating-point sums are order-independent. No worker pool or live music support is claimed yet.
