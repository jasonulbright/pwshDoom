...
param([Parameter(Mandatory)][string[]]$Names,[Parameter(Mandatory)][string]$Output,
    [ValidateSet('Output','Encoding')][string]$Dimension='Output')
...
try{
    ...
    if($Dimension -eq 'Encoding'){$expectedModes='Pairs','ColorState','ColorState','Pairs'}
    for($n=0;$n -lt $Names.Count;$n++){
        ...
        $actualMode=$g.TerminalOutput.Mode;$recordedMode=$c.TerminalOutput
        if($Dimension -eq 'Encoding'){$actualMode=$g.AnsiEncoding;$recordedMode=$c.AnsiEncoding}
        Check "$name uses the declared mode and retains every frame timestamp" ($actualMode -ceq $expectedModes[$n] -and $recordedMode -ceq $expectedModes[$n] -and $g.FrameStats.Count -eq $g.CompletedFrames)
        ...
        $encodeMax=@($level|ForEach-Object {($_.Workers|ForEach-Object {$_[1]}|Measure-Object -Maximum).Maximum})
        $renderMax=@($level|ForEach-Object {($_.Workers|ForEach-Object {$_[0]}|Measure-Object -Maximum).Maximum})
        $runs.Add(@{Name=$name;Mode=$actualMode;ConsoleWritesPerActiveSecond=$g.CompletedUpdatesPerSecond;TicsPerActiveSecond=$g.TicsPerSecond;
            ...
            LevelMaxWorkerEncodeMs=(Stats $encodeMax);LevelMaxWorkerRenderMs=(Stats $renderMax);
    ...
    @{Error=$failure;Dimension=$Dimension;DeclaredOrder=$expectedModes;Checks=$checks.ToArray();Runs=$runs.ToArray();SharedConfiguration=$reference;Receipts=$receipts.ToArray();SourceSha256=(Get-FileHash $PSCommandPath).Hash;
        Meaning='Four captured complete routes in the declared ABBA mode order. Same source/workload/audio/viewport/capture configuration and non-varied encoding/output mode verified; all frame observations retained. Level output and same-world gaps are explicit subsets. Two trials per mode expose variation but do not establish statistical confidence or universality. Console image completion is not distinct displayed FPS; capture is an added workload. Audio queue observations are not acoustic measurements.'}|ConvertTo-Json -Depth 10|Set-Content $Output
}
