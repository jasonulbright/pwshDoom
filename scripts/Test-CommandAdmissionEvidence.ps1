...
param([Parameter(Mandatory)][string]$Report,[Parameter(Mandatory)][string]$Replay,
    [Parameter(Mandatory)][string]$Recording,[Parameter(Mandatory)][string]$Output)
...
try{
    ...
    foreach($field in 'Skill','Episode','Map','ContinueCampaign','SourceFingerprint'){Check "Recorded $field matches input" ($expected.$field -eq $actual.$field)}
    ...
    Check 'Host independently reports matching sources and all checkpoints' ($reportData.ReplaySourceMatches -and $reportData.ReplayVerification.Matched -and $reportData.ReplayVerification.Checked -eq $expected.Checkpoints.Count)
    ...
    @{Error=$failure;Checks=$checks.ToArray();ReportSha256=(Get-FileHash $Report).Hash;ReplaySha256=(Get-FileHash $Replay).Hash;RecordingSha256=(Get-FileHash $Recording).Hash;
        HarnessSha256=(Get-FileHash $PSCommandPath).Hash;FinishedUtc=[DateTime]::UtcNow.ToString('o');
