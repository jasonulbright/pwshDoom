...
function Read-AnsiStripPixels {
    ...
    foreach($match in [regex]::Matches($Text,"$([char]27)\[([0-9;]+)([Hm])|$([char]0x2580)")){
        if($match.Groups[2].Value -eq 'H'){
            $coordinates=$match.Groups[1].Value.Split(';');$row=([int]$coordinates[0]-1-$RowOffset)*2;$column=[int]$coordinates[1]-1-$ColumnOffset
        ...
        foreach($workers in 1,3,7){
          foreach($origin in @(@(0,0),@(13,9))){
            ...
            for($i=0;$i -lt $workers;$i++){
                ...
                $bytes=ConvertTo-AnsiStrip $source 17 13 $first $end $context -ColumnOffset $origin[0] -RowOffset $origin[1]
            ...
            $actual=Read-AnsiStripPixels ([string]::Concat($parts)) 17 13 $terminalPalette -ColumnOffset $origin[0] -RowOffset $origin[1]
...
Write-Host "PASS: $cases ANSI strip round-trips, both origins/color modes, odd dimensions and uneven partitions."
