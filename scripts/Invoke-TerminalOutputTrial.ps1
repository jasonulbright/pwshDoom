...
try{
    ...
    $names=@(@($prior.Sources.Path)+@('src/TerminalOutput.ps1','scripts/Invoke-TerminalOutputTrial.ps1')|Sort-Object -Unique)
    @{CreatedUtc=[DateTime]::UtcNow.ToString('o');TerminalOutput=$Mode;CurrentSourceFingerprint=(Get-DoomReplaySourceFingerprint);
    ...
    ./scripts/Record-DoomReplay.ps1 -Style Classic -TerminalOutput $Mode -Replay $replay -OutputPrefix $prefix `
