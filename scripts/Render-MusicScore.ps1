...
param([Parameter(Mandatory)][string]$Output,[string]$Wad='C:\Program Files (x86)\Steam\steamapps\common\Ultimate Doom\base\DOOM.WAD',[string]$SoundFont="$PSScriptRoot/../local/upstream/ManagedDoomPowershell/ManagedDoomPowershell/src/TimGM6mb.sf2",
    [ValidatePattern('^D_[A-Z0-9]+$')][string]$Track='D_E1M1',[ValidateRange(1,300)][int]$Seconds=8,[ValidateRange(0,1)][double]$Volume=.2,[switch]$Profile)
$ErrorActionPreference='Stop';Set-StrictMode -Version Latest
if(Test-Path $Output){throw 'Use a fresh report path.'}
$sourcePaths=@('src/MusScore.ps1','src/SoundFontBank.ps1','src/SoundFontRegions.ps1','src/MusicOscillator.ps1','src/MusicControls.ps1','src/MusicSynth.ps1','src/AudioMixer.ps1','scripts/Render-MusicScore.ps1')
if($Profile){$sourcePaths+='scripts/Enable-MusicProfiler.ps1'}
$sourceHashes=@($sourcePaths|ForEach-Object {@{Path=$_;Sha256=(Get-FileHash (Join-Path "$PSScriptRoot/.." $_)).Hash}})
...
if($Profile){. "$PSScriptRoot/Enable-MusicProfiler.ps1"}
...
try{
    ...
    while($synth.Frame -lt $Seconds*44100){
        ...
        foreach($event in $block.Events){
            ...
            if($span -gt 0){
                if($Profile){$pfRead=[Diagnostics.Stopwatch]::GetTimestamp()};$mixed=Read-DoomMusicSynth $synth $span
                if($Profile){$synth.Profile.ReadTicks+=[Diagnostics.Stopwatch]::GetTimestamp()-$pfRead;$pfPcm=[Diagnostics.Stopwatch]::GetTimestamp()};$pcm=ConvertTo-DoomMusicPcm $synth $mixed
                if($Profile){$synth.Profile.PcmTicks+=[Diagnostics.Stopwatch]::GetTimestamp()-$pfPcm};$pcm.CopyTo($samples,$cursor);$cursor+=$pcm.Length
            }
            if($Profile){$pfEvent=[Diagnostics.Stopwatch]::GetTimestamp()};Invoke-DoomMusicEvent $synth $event
            if($Profile){$synth.Profile.EventTicks+=[Diagnostics.Stopwatch]::GetTimestamp()-$pfEvent}
        }
    ...
    @{Error=$failure;Details=$details;WadSha256=(Get-FileHash $Wad).Hash;Sources=$sourceHashes;SourcesChangedDuringRun=@($sourceHashes|Where-Object {$_.Sha256 -cne (Get-FileHash (Join-Path "$PSScriptRoot/.." $_.Path)).Hash});
