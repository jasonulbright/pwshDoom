...
$destination=@($g.Simulation.Transitions|Where-Object State -eq 'Level')[-1]
$mapImage=$prefix+"-e$($destination.Episode)m$($destination.Map).png"
foreach($p in $Output,($prefix+'-transitions.mp4'),($prefix+'-intermission.png'),$mapImage){if(Test-Path $p){throw 'Use fresh clip/image/report paths.'}}
$firstInter=@($g.FrameStats|Where-Object State -eq 1)[0];$firstMap2=@($g.FrameStats|Where-Object {$_.Generation -eq $destination.Generation -and $_.State -eq 0})[0]
...
try{
    ...
    & $ff -v error -n -ss (Number ($mapTime+.7)) -i $m.Video -frames:v 1 $mapImage
    ...
    @{Error=$failure;Video=$m.Video;VideoSha256=$m.VideoSha256;ViewingCopy=$prefix+'-transitions.mp4';ViewingCopySha256=if(Test-Path ($prefix+'-transitions.mp4')){(Get-FileHash ($prefix+'-transitions.mp4')).Hash}else{$null};
      ...
      DestinationEpisode=$destination.Episode;DestinationMap=$destination.Map;MapImagePath=$mapImage;
      Meaning='Actual movie excerpt around level exit, intermission and the final recorded destination generation, with sampled PNGs for separate visual review. Original full-window capture retained; no speed changes or discarded handoff time. Decode is not a visual or listening review.'}|ConvertTo-Json|Set-Content $Output
}
