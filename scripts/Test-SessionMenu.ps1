...
try{
    ...
    $null=Invoke-DoomMenuKey $m Up;Assert-Menu 'Choice wraps upwards' ($m.Choice -eq 3)
    ...
    $directory="$PSScriptRoot/../local/menu-unit-frames";[void][IO.Directory]::CreateDirectory($directory)
    for($screen=1;$screen -le 7;$screen++){
        $count=switch($screen){1{4};2{4};3{5};4{2};6{2};default{1}}
        for($choice=0;$choice -lt $count;$choice++){
            $frame=Get-DoomMenuPixels $graphics $screen $choice 4 5 4
    ...
    @{FinishedUtc=[DateTime]::UtcNow.ToString('o');Error=$failure;Checks=$checks.ToArray();Frames=$frames.ToArray();WadSha256=(Get-FileHash $Wad).Hash;
