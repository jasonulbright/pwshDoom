...
function Submit-GameRender {
    param($Pool,$Snapshot,[int]$ColumnOffset=0,[int]$RowOffset=0,[int]$FrameNumber=0,[switch]$ScreenPixels,[int]$Tic=0,[switch]$MenuPixels)
    ...
    if(($ScreenPixels -or $MenuPixels) -and $bytes.Length -ne 64000){throw 'Session screens require 320x200 indexed pixels.'}
    foreach($worker in $Pool.Workers) {
        ...
        $worker.View.Write(76,$(if($MenuPixels){3}else{[int][bool]$ScreenPixels}));$worker.View.Write(80,$Tic);[void]$worker.Go.Set()
    }
}
