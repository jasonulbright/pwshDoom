...
function New-DoomMenuState {
    ...
    return @{Screen=0;Choice=0;Episode=$Episode;Skill=$Skill;Episodes=$Episodes;Slots=@(1..6|ForEach-Object {@{Slot=$_;State='Empty';Sha256=$null;Episode=0;Map=0;Skill=0;Time='';SourceMatches=$true}});SelectedSlot=1;MessageTitle='';MessageDetail='';ReturnScreen=1}
}
function Invoke-DoomMenuKey {
    ...
    if($screen -eq 0){
        ...
        $count=switch($screen){1{6};2{$Menu.Episodes};3{5};8{6};9{6};default{2}}
        ...
        switch($screen){
            1 {switch($Menu.Choice){0{$Menu.Screen=0};1{$Menu.Screen=if($Menu.Episodes -gt 1){2}else{3};$Menu.Choice=if($Menu.Screen -eq 2){$Menu.Episode-1}else{$Menu.Skill-1}};2{$Menu.Screen=8;$Menu.Choice=0};3{$Menu.Screen=9;$Menu.Choice=0};4{$Menu.Screen=5};5{$Menu.Screen=6;$Menu.Choice=0}}}
            ...
            6 {if($Menu.Choice -eq 1){return @{Action='Quit'}}else{$Menu.Screen=1;$Menu.Choice=5}}
            {$_ -in 8,9} {
    ...
    return @{Action='ShowMenu';Screen=$Menu.Screen;Choice=$Menu.Choice;Episode=$Menu.Episode;Skill=$Menu.Skill;Slots=$Menu.Slots;SelectedSlot=$Menu.SelectedSlot;MessageTitle=$Menu.MessageTitle;MessageDetail=$Menu.MessageDetail}
}
...
function Get-DoomCompactMenu {
    ...
    $title=switch($Menu.Screen){1{'pwshDoom menu'};2{'Choose episode'};3{'Choose skill'};4{'Start a new game?'};5{'Controls'};6{'Quit Doom?'};7{'Paused'};8{'Save game'};9{'Load game'};10{'Replace this save?'};11{'Load this save?'};12{$Menu.MessageTitle};13{$Menu.MessageTitle};default{'pwshDoom'}}
    $items=switch($Menu.Screen){
        1 {@('Resume game','New game','Save game','Load game','Controls','Quit')}
...
function Get-DoomMenuPixels {
    ...
    switch($Screen){
        1 {
            ...
            $labels=@('RESUME GAME','NEW GAME','SAVE GAME','LOAD GAME','CONTROLS','QUIT')
            for($i=0;$i -lt $labels.Count;$i++){Draw-DoomMenuText $Graphics $labels[$i] 66 (64+16*$i) 2}
            $draw.DrawPatch($patches.M_SKULL1,36,(62+16*$Choice),1)
        }
        2 {
