# pwshDoom modification, 2026-09-10: optional strip bounds for reference-renderer experiments.
##
...
class DrawScreen {
    [int] $Width
    [int] $Height
    # pwshDoom, 2026-09-10: disjoint terminal-worker output columns.
    [int] $FirstColumn = 0
    [int] $EndColumn
    [byte[]] $Data
    [Patch[]] $Chars
    ...
    [void] DrawPatch([Patch] $patch, [int] $x, [int] $y, [int] $scale) {
        ...
        if ($drawX -lt $this.FirstColumn) {
            $exceed = $this.FirstColumn - $drawX
            $i += $exceed
        }

        if ($drawX + $drawWidth -gt $this.EndColumn) {
            $exceed = $drawX + $drawWidth - $this.EndColumn
            $drawWidth -= $exceed
        }
