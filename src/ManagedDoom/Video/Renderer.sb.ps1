...
class Renderer {
    ...
    [void] WriteData([uint[]] $colors, [byte[]] $destination) {
        # pwshDoom modification, 2026-09-10: remove compiled BufferHelper.
        # The terminal backend uses indexed Screen.Data directly. Retain this
        # optional RGBA conversion as PowerShell for callers of Render().
        $screenData = $this.screen.Data
        for ($x=0; $x -lt $this.Screen.Width; $x++) {
            for ($y=0; $y -lt $this.Screen.Height; $y++) {
                $rgba=$colors[$screenData[$x*$this.Screen.Height+$y]]
                $offset=4*($y*$this.Screen.Width+$x)
                $destination[$offset]=[byte]($rgba -band 255)
                $destination[$offset+1]=[byte](($rgba -shr 8) -band 255)
                $destination[$offset+2]=[byte](($rgba -shr 16) -band 255)
                $destination[$offset+3]=[byte](($rgba -shr 24) -band 255)
            }
        }
    }
