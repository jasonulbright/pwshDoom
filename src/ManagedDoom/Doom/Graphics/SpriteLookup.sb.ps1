# pwshDoom modification, 2026-09-10: use sprite names and exclude the enum Count sentinel.
##
    ...
    SpriteLookup([Wad]$wad) {
        try {
            ...
            for ($spriteIndex = 0; $spriteIndex -lt $spriteValues.Count; $spriteIndex++) {
                ...
                # pwshDoom, 2026-09-10: exclude the enum Count sentinel.
                if ([int]$sprite -ge [DoomInfo]::SpriteNames.Names.Length) { continue }
                $key = [DoomInfo]::SpriteNames.Names[[int]$sprite].Replaced
                if ($null -ne $key) {
            ...
            $this.spriteDefs = New-Object SpriteDef[] ([DoomInfo]::SpriteNames.Names.Length)
            for ($i = 0; $i -lt $this.spriteDefs.Length; $i++)
            {
                $spriteName = [DoomInfo]::SpriteNames.Names[$i].Replaced
