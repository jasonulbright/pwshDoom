# pwshDoom modification, 2026-09-10: qualify instance string references.
##
...
class Strings {
        ...
        [doomstring]$LOADNET = [DoomString]::new("LOADNET", "you can't do load while in a net game!`n`n" + $this.PRESSKEY.Replaced)
        [doomstring]$QLOADNET = [DoomString]::new("QLOADNET", "you can't quickload during a netgame!`n`n" + $this.PRESSKEY.Replaced)
        [doomstring]$QSAVESPOT = [DoomString]::new("QSAVESPOT", "you haven't picked a quicksave slot yet!`n`n" + $this.PRESSKEY.Replaced)
        [doomstring]$SAVEDEAD = [DoomString]::new("SAVEDEAD", "you can't save if you aren't playing!`n`n" + $this.PRESSKEY.Replaced)
        [doomstring]$QSPROMPT = [DoomString]::new("QSPROMPT", "quicksave over your game named`n`n'%s'`n`n" + $this.PRESSYN.Replaced)
        [doomstring]$QLPROMPT = [DoomString]::new("QLPROMPT", "do you want to quickload the game named`n`n'%s'`n`n" + $this.PRESSYN.Replaced)
        [doomstring]$NEWGAME = [DoomString]::new("NEWGAME", "you can't start a new game`nwhile in a network game.`n`n" + $this.PRESSKEY.Replaced)
        [doomstring]$NIGHTMARE = [DoomString]::new("NIGHTMARE", "are you sure? this skill level`nisn't even remotely fair.`n`n" + $this.PRESSYN.Replaced)
        [doomstring]$SWSTRING = [DoomString]::new("SWSTRING", "this is the shareware version of doom.`nyou need to order the entire trilogy.`n`n" + $this.PRESSYN.Replaced)
        [doomstring]$MSGOFF = [DoomString]::New("MSGOFF", "Messages OFF")
        [doomstring]$MSGON = [DoomString]::New("MSGON", "Messages ON")
        [doomstring]$NETEND = [DoomString]::New("NETEND", "you can't end a netgame!`n`n" + $this.PRESSKEY.Replaced)
        [doomstring]$ENDGAME = [DoomString]::New("ENDGAME", "are you sure you want to end the game?`n`n" + $this.PRESSYN.Replaced)
