...
class GameContent {
    ...
    GameContent($gameArguments) {
        # pwshDoom modification, 2026-09-10: these consumers parse raw arguments
        # themselves; passing a CommandLineArgs object loses the explicit IWAD.
        $this.Wad = [Wad]::new([ConfigUtilities]::GetWadPaths($gameArguments))

        [DeHackEd]::Initialize($gameArguments, $this.Wad)
