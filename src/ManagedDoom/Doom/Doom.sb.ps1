...
class Doom {
    ...
    Doom($gameArguments, [Config] $config, [GameContent] $content, [IVideo] $video, [ISound] $sound, [IMusic] $music, [IUserInput] $userInput) {
        if ($video) {
            $this.Video = $video
        } else {
        ...
        $this.Args = [CommandLineArgs]::new($gameArguments)
        $this.Config = $config
        ...
        $this.Options.GameOptionsArgs($gameArguments,$content)
