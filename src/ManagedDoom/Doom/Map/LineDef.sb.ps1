...
class LineDef {
    ...
    LineDef(
        ...
        # pwshDoom, 2026-09-10: one-sided lines have no back sidedef.
        $this.frontSector = if ($null -ne $frontSide) { $frontSide.Sector } else { $null }
        $this.backSector = if ($null -ne $backSide) { $backSide.Sector } else { $null }
    }
