...
class Hitscan {
    ...
    [Mobj] $LineTarget
    [Mobj] $CurrentShooter
    [Fixed] $CurrentShooterZ = [Fixed]::Zero
    [Fixed] $CurrentRange = [Fixed]::Zero
    [Fixed] $CurrentAimSlope = [Fixed]::Zero
    [int] $CurrentDamage
    ...
    [bool] ShootTraverse([Intercept] $intercept) {
        ...
        if ($null -ne $intercept.Line) {
            ...
            if ($line.FrontSector.CeilingFlat -eq $this.world.Map.Flats.SkyFlatNumber) {
                # Don't shoot the sky!
                ...
                if ($null -ne $line.BackSector -and $line.BackSector.CeilingFlat -eq $this.world.Map.Flats.SkyFlatNumber) {
                    return $false
                }
            }
