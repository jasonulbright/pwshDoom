...
param([Parameter(Mandatory)][string]$Replay,[Parameter(Mandatory)][string]$Output,[switch]$Profile,
...
if($Profile){
    ...
    $source=Replace-Once $source 'class World {' ('class World {'+"`n"+'    [long[]]$ProfileTicks=[long[]]::new(9)'+"`n")
...
try{
    ...
    @{Error=$failure;Profile=[bool]$Profile;Commands=$completed;FinishedUtc=[DateTime]::UtcNow.ToString('o');PowerShell=$PSVersionTable.PSVersion.ToString();
