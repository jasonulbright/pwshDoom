...
if($Profile){
    ...
    $source=Replace-Once $source 'class World {' ('class World {'+"`n"+'    [long[]]$ProfileTicks=[long[]]::new(9)'+"`n"+'    [long[]]$ProfileActorTicks=[long[]]::new(2)'+"`n"+'    [long[]]$ProfileActorCounts=[long[]]::new(4)'+"`n")
    if($ActorProfile){
        $index=0
...
try{
    ...
    for($i=0;$i -lt [Math]::Min($MaxCommands,$reference.InputCommands.Count);$i++){
        ...
        if($ActorProfile){
            ...
            for($stage=0;$stage -lt 2;$stage++){$sample.ActorMilliseconds+=($game.World.ProfileActorTicks[$stage]-$(if($sample.MapChanged){0}else{$beforeActorTicks[$stage]}))*1000.0/[Diagnostics.Stopwatch]::Frequency}
            for($stage=0;$stage -lt 4;$stage++){$sample.ActorCounts+=($game.World.ProfileActorCounts[$stage]-$(if($sample.MapChanged){0}else{$beforeActorCounts[$stage]}))}
        }
        $samples.Add($sample)
    ...
    @{Error=$failure;Profile=[bool]$Profile;ActorProfile=[bool]$ActorProfile;ActorArrayOrder=@('XY','Z');ActorCountOrder=@('XYCalls','ZCalls','NumericallyUnneededXY','NumericallyUnneededZ');Commands=$completed;FinishedUtc=[DateTime]::UtcNow.ToString('o');PowerShell=$PSVersionTable.PSVersion.ToString();
